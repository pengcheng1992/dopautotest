...
from pagelib.baselib import HtmlLogger
...
class BaseTool(object):
    ...
    def open_browser(self,url,msg=None):
        ...
        if  (sc.browser == "chrome"):
            ...
            try:
                ...
                self.log.error("打开谷歌浏览器失败")
        elif (sc.browser == "firefox"):
            # 打开火狐浏览器
            try:
                self.driver = webdriver.Firefox()
                self.log.info("打开火狐浏览器")
            except:
                self.log.error("打开火狐浏览器失败")
        elif (sc.browser == "ie"):
            # 打开IE浏览器
            try:
                self.driver = webdriver.Ie()
                self.log.info("打开IE浏览器")
            except:
                self.log.error("打开IE浏览器失败")
        else:
            self.log.error("不支持的浏览器")
        self.max_window()
        # 跳转到url
        self.get_url(url)
    ...
    def get_url(self,url):
        try:
            self.driver.get(url)
            self.log.info("跳转到：【"+url+"】页面")
        except:
            self.log.error("跳转【"+url+"】失败，请检查URL是否正确！")

    # 关闭浏览器
    def close_browser(self):
        try:
            self.driver.close()
            self.log.info("成功关闭浏览器")
        except:
            self.log.error("关闭浏览器失败")
    ...
    def pic_shot(self):
        try:
            ...
            self.log.debug("截图路径：【"+filepath+"】")
            op.path_exists_create(filepath)
            # 截图名字
            filename = gt.get_time_filename()+lc.shot_type
            self.log.debug("截图名字：【"+filename+"】")
            name = filepath+filename
            self.log.debug("截图完整路径+名字：【"+name+"】")
            # 进行截图操作
            self.driver.save_screenshot(name)
        except :
            self.log.error("截图失败")
        else:
            self.log.info("截图成功,截图存储路径为：【"+name+"】")
            self.log.info("picshot_1"+showpath+filename)
    ...
    def get_data(self,data_dict,env=None):
        ...
        if env == None and sc.env in sc.envlist:
            try:
                ...
                self.log.debug("获取到的数据为：\n"+value)
                return value
            except:
                self.log.error("不支持的数据格式，请使用python字典格式！")
            ...
            self.log.error("环境配置不正确，请检查环境配置，仅支持【qa】、【yz】、【prod】三种环境！")
